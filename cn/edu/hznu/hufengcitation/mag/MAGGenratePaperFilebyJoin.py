...
sql_str_pubyear_of_all_papers_join="select a.id,a.pubyear from papers a inner join paperkeywords b on a.id=b.paperid where b.fieldid='%s' order by id ASC "
...
#            continue
        tmp_sql_str_pubyear_of_all_papers_join= sql_str_pubyear_of_all_papers_join % subfield
 #       logger.info(tmp_sql_str_papers_of_all_subfields)
 #       exit()
        all_papers= []
        all_papers = db.get_query_results(tmp_sql_str_pubyear_of_all_papers_join)
        if(len(all_papers)==0):
            continue
#        logger.info(tmp_sql_str_pubyear_of_all_papers_in)
#        exit()
...
#             logger.info('Get paper pubyears.  root:%d, # of paper pubyears: %d time cost:%d', rootcount, len(pubyear_of_all_papers),
        ...
        for p in all_papers:
            papers= papers+ ("%s\t%s\n" % (p[0],p[1]))
        '''
              tmp_sql_str_citations_of_all_papers = sql_str_citations_of_all_papers % paper
              citation_of_all_papers = []
              citation_of_all_papers = db.get_query_results(tmp_sql_str_citations_of_all_papers)
              for c in citation_of_all_papers:
                  citations = citations + ("%s\t%s\n" % (c[0], c[1]))
      '''
        f_dest.write(papers)
#        f_dest_citation.write(citations)
        papers=None
#        citations=None
        str_subfield="%s" % subfield
        time_end = time.time()
        logger.info('Subfieleds Generated. Root:%d, #subfields:, %d/%d:%f%%, fields:%s, #papers:%d, time cost:%d',rootcount, num_subfields_of_one_root,len(subfields_of_one_root), num_subfields_of_one_root*1.0/len(subfields_of_one_root)*100,str_subfield,len(all_papers),time_end - time_start)
