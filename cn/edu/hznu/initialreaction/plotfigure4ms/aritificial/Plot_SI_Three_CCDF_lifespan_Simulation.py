...
data_observation= ["Random", "Small_World", "Scale_Free"]
...
time_labels=['4','8','12',100]
time_positions=[4, 8,12,100]
...
def shaded_Error_Bar_Mean_Error_Params_SubPlot_OneCaption(x, y, subplot_pos,type, pars=[], logx=False,logy=False, n=1, isSave=False):
    ...
    if (pars[0] is not None):
        ...
        plt.text(xlabel_axis[0], xlabel_axis[1], xlabel,  transform=axes.transAxes,
                 color='black', size='10', weight="light")
    if (isSave == False):
        plt.text(0.45, 2.05, indicators[type], fontsize=15)
    else:
        plt.text(0.55, 3.8, indicators[type], fontsize=15)
...
#                  family="fantasy", size='8', color = 'black', weight = "light")
    ...
    if(isSave==True):
        y2 = 0.003
    axes.text(x2, y2, str_fig_type[type], \
...
for data_obs in data_observation:
    ...
    num_data_count=0
    isSave = False
    logger.info('plotting lifespan vs timesteps: %s' % data_obs)
    legend=data_obs
...
#    logger.info(tmp_values_y)
    ...
    x_num_show=True
    y_num_show=True
    subplot="%s,%s,%s" % (len(data_observation),len(data_observation), num_data_dir+1)
    fig_file=""
    #logger.info(subplot)
