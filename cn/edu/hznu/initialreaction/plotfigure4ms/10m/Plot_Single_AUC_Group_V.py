...
fig_file = "D:\\py\\data\\initialreaction\\figs\\AUC.png"


#y=[1,2,3,4,5,6]#给出在y轴上的位置
y=[1,2,3,4,5,6]#给出在y轴上的位置
label=['All','Initial Attention \n(1h)  ','Initial Attention \n(30m)  ','Initial Attention \n(10m) ','Initial Time','Initial Structure']#直方图信息
#label=['All','Initial Attention (1h)  ','Initial Attention (30m)  ','Initial Attention (10m) ','Initial Time','Initial Structure']#直方图信息
colors=['tan','red','pink','orange','blue','green']
...
for num in range(len(x_weibo)):
  l=length-num-1
  x.append([x_weibo[l],x_twitter[l]])
  x_weibo_reverse.append(x_weibo[l])
  x_twitter_reverse.append(x_twitter[l])
  labels_small_big.append(label[l])


str = ["Weibo", "Twitter"]
df = pd.DataFrame(x)
df = pd.DataFrame([x_weibo_reverse,x_twitter_reverse],columns=labels_small_big,index=str)
print(df)
...
width =0.5
df.plot.bar(width=width,alpha=0.7,rot=0,tick_label = str,color=colors)#绘制水平直方图

plt.tick_params(axis='x', tickdir=None,length=0)
#plt.tick_params(axis='y', tickdir='out', labelsize=5, length=0)z
plt.legend(ncol=1, fontsize=8,bbox_to_anchor=(0.63,0.6),borderaxespad = -0.2, frameon=False)
plt.xlim(-0.3,1.3)
plt.text(-0.5,0.5,"AUC", rotation=90,fontsize=12)
