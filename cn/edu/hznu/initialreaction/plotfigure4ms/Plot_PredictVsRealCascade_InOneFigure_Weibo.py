...
#    ax.set_yticks([])
    ...
    axes.scatter(x,y,s=line_width, color=colors[1])
...
#        subplot=int("%s%s%s" % (len(data_observation),len(data_predict[0]), num_data_dir*(len(data_predict[0]))+num_data_count+1))
        ...
        fig_file=""
        #logger.info(subplot)
        if (num_data_dir==len(data_observation) and (num_data_count==len(data_predict[0]))):
           isSave=True
           fig_file = (fig_data % (str_data_type+".png"))
           xlabel = xlabel_bak
           ylabel = ylabel_bak
        pars=[xlabel,ylabel,1,colors[0],fig_file,legend,num_data_type,x_text_axis,y_text_axis]
