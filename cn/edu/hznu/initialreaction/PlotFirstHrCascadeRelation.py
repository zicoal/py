...
fig_file = dir+"output\\Hr_level_size_first_%s.png"
...
#                dist= nx.shortest_path_length(g,node,chk_id0)
                if( not user_top_hr_index.get(node[0]) is  None):
                    ...
                    level = nx.shortest_path_length(g, chk_id0, node[0])
#                    num_hr_level[hr] += level
#                    num_hr_count[hr] += 1
#                    num_hr_cascade_size[hr] += len(retweets)
                    list_hr_level.setdefault(hr, []).append(level)
                    list_hr_cascade_size.setdefault(hr, []).append(len(retweets))
                    max_retweet_count+=1
...
ax1.plot(x, y1,'o-',label=('Layer'),color=colors[1], linewidth='2')
...
ax2.plot(x, y2,'o-',label=('Cascade Size'),color=colors[3], linewidth='2')
