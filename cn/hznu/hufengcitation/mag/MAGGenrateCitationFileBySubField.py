...
dir="/home/zico/mag/output/allfields/citation/"
#dir="D:\\data\\MAG\\output\\"
time_start=time.time()

dest_file= dir + "citation_%s.txt"
#dest_citation_file= dir + "citing_%s_%s.txt"

sql_str_subfield="select distinct id from fieldswithroot order by id ASC "
sql_str_citations_of_all_subfields= "select paperid,refid from paperreferenceswithfield where paperfieldid='%s' or reffieldid='%s' order by refid ASC "
...
time_end=time.time()
logger.info('Done: Read SubFields from DB! Time cost:%d s', time_end - time_start)
...
#        continue
    subf = ("%s" %field)
    dest_file_field = dest_file % subf
...
#    f_dest_citation = open(dest_citation_file_root, encoding='UTF-8', mode='w', errors='ignore')
    ...
    tmp_sql_str_papers_of_all_subfields = sql_str_citations_of_all_subfields % (subf, subf)
    all_citations = []
    all_citations = db.get_query_results(tmp_sql_str_papers_of_all_subfields)

    citations = ''
    for p in all_citations:
        citations = '%s%s\t%s\n' % (citations, p[0], p[1])
    f_dest.write(citations)
    citations=None
    time_end = time.time()
    logger.info('Subfiled Generated. #subfields: %s, %d/%d:%f%%, #citations:%d, time cost:%d', ("%s" % field), num_subfields_of_all_roots, len(fields), num_subfields_of_all_roots * 1.0 / len(fields) * 100, len(all_citations), time_end - time_start)
    f_dest.close()
